  ...
  try {
    ...
    const pageTimings =
      ...
      typeof job.result.page_timings_ms === "object"
        ? { ...job.result.page_timings_ms }
        : {};
    const pageInputBytes =
      job.result?.page_input_bytes &&
      typeof job.result.page_input_bytes === "object"
        ? { ...job.result.page_input_bytes }
        : {};
    const pageModels =
      job.result?.page_models &&
      typeof job.result.page_models === "object"
        ? { ...job.result.page_models }
        : {};

    for (const success of successes) {
      pageTimings[String(success.pageNo)] = success.durationMs;
      pageInputBytes[String(success.pageNo)] = success.inputBytes;
      pageModels[String(success.pageNo)] = success.model;
    }
