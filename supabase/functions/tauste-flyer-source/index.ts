...
function fullViewUrl(publication:any) {
  ...
  return PROFILE_URL + direct.replace(/\.html(?:$|[?#])/i, "/full-view.html");
}
...
async function profilePublications() {
  ...
  return data.filter((publication:any)=>{
    ...
    return name.includes("ofertas tauste") &&
      name.includes("marilia") &&
      collectionHash(publication);
  });
}

async function resolvePublication(publication:any) {
  ...
  if (!dataResponse.ok) throw new Error("DATA_HTTP_"+dataResponse.status);
  ...
  const order=Array.isArray(data?.pages?.order) ? data.pages.order.flat() : [];
  const pages=order.map((pageId:unknown,index:number)=>{
    ...
    return {
      ...
      url:
        "https://d3u72tnj701eui.cloudfront.net/"+ACCOUNT_ID+
        "/collections/"+hash+"/covers/"+encodeURIComponent(id)+
...
Deno.serve(async(req:Request)=>{
  ...
  try {
    ...
    for (const publication of publications) {
      flyers.push(await resolvePublication(publication));
    }
    return json(200,{
      retailer:"Tauste",
      city:"Marília",
