...
declare
  ...
  v_pork_pattern text :=
    '^(carne suin[oa]s?|bisteca( da paleta)? suin[oa]|bisteca .* suin[oa]|file mignon suin[oa]|costela suin[oa]|costelinha suin[oa]|lombo|copa lombo( suin[oa])?|paleta suin[oa]|panceta( suin[oa])?|pernil( suin[oa])?|orelha suin[oa]|pe suin[oa]|pele suin[oa]|rabo suin[oa])( |$)';
  v_non_pork_pattern text :=
  ...
  v_fish_pattern text :=
    '^((peixe|peixes|pescado|pescados)( |$)|(file|files) (de )?(peixe )?(tilapia|merluza|linguado|panga|salmao|atum|sardinha|bacalhau|pacu|tambaqui|pintado|corvina|tainha|anchova|truta|pescada|pescadinha|cacao)( |$)|file (tilapia|merluza|linguado|panga|salmao|atum|sardinha|bacalhau|pacu|tambaqui|pintado|corvina|tainha|anchova|truta|pescada|pescadinha|cacao)( |$)|posta (de )?(tilapia|merluza|linguado|panga|salmao|atum|sardinha|bacalhau|pacu|tambaqui|pintado|corvina|tainha|anchova|truta|pescada|pescadinha|cacao)( |$)|costelinha (de )?(pacu|tambaqui)( |$)|(tilapia|merluza|linguado|panga|salmao|atum|sardinha|bacalhau|pacu|tambaqui|pintado|corvina|tainha|anchova|truta|pescada|pescadinha|cacao)( |$))';

  v_processed_meat_pattern text :=
...
begin
  ...
  v_alias_query := trim(
    regexp_replace(' ' || v_query || ' ', ' +(de|da|do|das|dos) +', ' ', 'g')
  );
  ...
  if v_tsquery is null and v_family is null then
    return;
  end if;
  ...
  with active_rows as (
    select
      ...
      true as is_active,
      case
        when v_family is not null then 1::real
        else ts_rank_cd(
          to_tsvector(
            'simple',
    ...
    where fi.user_id = v_uid
      ...
      and (
        (
          ...
          and coalesce(fi.normalized_name, '') !~ v_processed_meat_pattern
        )
        or (
          v_family = 'pork'
          and coalesce(fi.normalized_name, '') ~ v_pork_pattern
          and coalesce(fi.normalized_name, '') !~ v_non_pork_pattern
          and coalesce(fi.normalized_name, '') !~ v_processed_meat_pattern
        )
        or (
          v_family = 'fish'
          and coalesce(fi.normalized_name, '') ~ v_fish_pattern
        )
        or (
          v_family is null
          and to_tsvector(
