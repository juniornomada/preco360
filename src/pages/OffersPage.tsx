...
function productHeadRule(
  ...
  if (family === "tomatoSauce") {
    return {
      ...
      pattern: /^molho(?:\\s+de)?\\s+tomate\\b/i,
    };
  }
  if (family === "tomatoExtract") {
    return {
      label: "Extrato de Tomate",
      pattern: /^extrato(?:\\s+de)?\\s+tomate\\b/i,
    };
  }
  if (family === "tomatoPassata") {
    return {
      label: "Passata de Tomate",
      pattern: /^passata(?:\\s+de)?\\s+tomate\\b/i,
    };
  }
  if (family === "tomatoPeeled") {
    return {
      label: "Tomate Pelado",
      pattern: /^tomate(?:s)?\\s+pelad[oa]s?\\b/i,
    };
  }
  if (family === "milkLiquid") {
    return { label: "Leite", pattern: /^leite\\b/i };
  }
  if (family === "milkPowder") {
    return {
      label: "Leite em Pó",
      pattern: /^leite(?:\\s+em)?\\s+p[oó]\\b/i,
    };
  }
  if (family === "milkCream") {
    return {
      label: "Creme de Leite",
      pattern: /^(?:mistura\\s+de\\s+)?creme\\s+de\\s+leite\\b/i,
    };
  }
  if (family === "milkCondensed") {
    return {
      label: "Leite Condensado",
      pattern: /^leite\\s+condensado\\b/i,
    };
  }
  if (family === "milkFermented") {
    return {
      label: "Leite Fermentado",
      pattern: /^leite\\s+fermentado\\b/i,
    };
  }
  if (family === "milkCoconut") {
    return {
      label: "Leite de Coco",
      pattern: /^leite\\s+de\\s+coco\\b/i,
    };
  }
  if (family === "milkSweet") {
    return {
      label: "Doce de Leite",
      pattern: /^doce\\s+de\\s+leite\\b/i,
    };
  }
  if (family === "coffeeGround") {
    return { label: "Café", pattern: /^caf[eé]\\b/i };
  }
  if (family === "coffeeBeans") {
    return {
      label: "Café em Grãos",
      pattern: /^caf[eé](?:\\s+em)?\\s+gr[aã]os?\\b/i,
    };
  }
  if (family === "coffeeSoluble") {
    return {
      label: "Café Solúvel",
      pattern: /^caf[eé]\\s+sol[uú]vel\\b/i,
    };
  }
...
function standardizedOfferName(item: FlyerItemRow, family: OfferFamily) {
  ...
  const withoutBrand = cleanDisplayNamePart(
    compact.replace(brandPattern, " "),
  );
  ...
  let remainder = cleanDisplayNamePart(
    withoutBrand.replace(headRule.pattern, " "),
  );

  if (
    family === "tomatoSauce" &&
    /^molho\\s+tomate\\b/i.test(withoutBrand)
  ) {
